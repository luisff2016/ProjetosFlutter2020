...
class AlergiasDB {
  ...
  Future get database async {
    try {
      ...
      print("##45 alergia AlergiasDB.get-database(): _db = $_db");
      return _db;
    } catch (e) {
  ...
  Future<Database> init() async {
    print("## alergia AlergiasDB.init()");
    String path = join(utils.docsDir.path, "alergias.db");
    print("##47 alergia AlergiasDB.init(): path = $path");
  ...
  Future create(Alergia inAlergia) async {
    print("## 52 alergia AlergiasDB.create(): inAlergia = $inAlergia");
  ...
  Future<Alergia> get(int inID) async {
    print("## 53 alergia AlergiasDB.get(): inID = $inID");
    Database db = await database;
    var rec = await db.query("alergias", where: "id = ?", whereArgs: [inID]);
    print("## 54 alergia AlergiasDB.get(): rec.first = $rec.first");
  ...
  Future update(Alergia inAlergia) async {
    print("## 58 alergia AlergiasDB.update(): inAlergia = $inAlergia");
    try {
      Database db = await database;
      return await db.update("alergias", alergiaToMap(inAlergia),
          where: "id = ?", whereArgs: [inAlergia.id]);
    } catch (e) {
      print("## 59 alergia ERRO AlergiasDB.update(): $e ");
    }
  } /* End update(). */
  ...
  Future delete(int inID) async {
    print("## 60 alergia AlergiasDB.delete(): inID = $inID");
    try {
      Database db = await database;
      return await db.delete("alergias", where: "id = ?", whereArgs: [inID]);
    } catch (e) {
      print("## 61 alergia ERRO AlergiasDB.delete(): $e ");
    }
  } /* End delete(). */
