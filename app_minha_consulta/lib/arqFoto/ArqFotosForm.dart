...
/// ********************************************************************************************************************
/// The ArqFotos Entry sub-screen.
/// ********************************************************************************************************************
class ArqFotosForm extends StatelessWidget {
  ...
  ArqFotosForm() {
    ...
    print("##105 ArqFotosForm.constructor");

    // Attach event listeners to controllers to capture entries in model.
    _titleEditingController.addListener(() {
      arqFotosModel.entidadeSendoEditada.title = _titleEditingController.text;
    });

    _descriptionEditingController.addListener(() {
      arqFotosModel.entidadeSendoEditada.description = _descriptionEditingController.text;
    });
  ...
  Widget build(BuildContext inContext) {
    ...
    print("##106 ArqFotosForm.build()");

    // Set value of controllers.
    _titleEditingController.text = arqFotosModel.entidadeSendoEditada.title;

    _descriptionEditingController.text = arqFotosModel.entidadeSendoEditada.description;
            ...
            bottomNavigationBar : Padding(
              ...
              child : Row(
                children : [
                  ...
                  FlatButton(
                    child : Text("Save"),
                    onPressed : () { _save(inContext, arqFotosModel); }
                  )
                ]
              )
            ),
              ...
              child : ListView(
                children : [
                  ...
                  ListTile(
                    ...
                    title : Text("Date"),
                    subtitle : Text(arqFotosModel.dataEscolhida == null ? "" : arqFotosModel.dataEscolhida),
                    trailing : IconButton(
                      ...
                      onPressed : () async {
                        ...
                        String dataEscolhida = await utils.selectDate(
                          inContext, arqFotosModel, arqFotosModel.entidadeSendoEditada.apptDate
                        );
                        if (dataEscolhida != null) {
                          arqFotosModel.entidadeSendoEditada.apptDate = dataEscolhida;
                        }
                      }
                    )
                  ),
                  ...
                  ListTile(
                    ...
                    title : Text("Time"),
                    subtitle : Text(arqFotosModel.apptTime == null ? "" : arqFotosModel.apptTime),
                    trailing : IconButton(
                      icon : Icon(Icons.edit),
                      color : Colors.blue,
  ...
  Future _selectTime(BuildContext inContext) async {
    ...
    if (picked != null) {
      arqFotosModel.entidadeSendoEditada.apptTime = "${picked.hour},${picked.minute}";

      arqFotosModel.setApptTime(picked.format(inContext));
    }
  ...
  ///
  /// @param inContext The BuildContext of the parent widget.
  /// @param inModel   The ArqFotosModel.
  void _save(BuildContext inContext, ArqFotosModel inModel) async {

      print("##107 ArqFotosForm._save()");
      ...
      if (inModel.entidadeSendoEditada.id == null) {
        ...
        print("##108 ArqFotosForm._save(): Creating: ${inModel.entidadeSendoEditada}");
        await ArqFotosDB.db.create(arqFotosModel.entidadeSendoEditada);

      // Updating an existing appointment.
      } else {

        print("##109 ArqFotosForm._save(): Updating: ${inModel.entidadeSendoEditada}");
        await ArqFotosDB.db.update(arqFotosModel.entidadeSendoEditada);
